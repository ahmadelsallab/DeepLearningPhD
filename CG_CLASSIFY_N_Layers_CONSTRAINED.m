...
% method for only top N layers. N is
...
% the weigths
function [f, df] = CG_CLASSIFY_N_Layers_CONSTRAINED(VV, Dim, wInputProbs, target, wInputProbs_prev, BP_prevWeights, w_class_prev, alpha, eMappingMode, NW_unitWeights, NW_weights_in, w_class_in);
...
wInputProbs_prev = [wInputProbs_prev ones(N,1)];
BP_layerInputData = wInputProbs_prev;
for(layer = 1 : N_layers)
...
targetout_prev = exp(BP_prevWprobs{N_layers}*w_class_prev);
